...
module.exports = async (callback) => {
  ...
  ({ expectedRate, slippageRate } = await NetworkProxyInstance.getExpectedRate(
    ETH_ADDRESS, // srcToken
    BAX.address, // destToken
    web3.utils.toWei(new BN(1)), // srcQty
  ));
  stdlog(
    `ETH <-> BAX getExpectedRate() = expectedRate: ${expectedRate}, slippageRate:${slippageRate}`,
  );

  ({ expectedRate, slippageRate } = await NetworkProxyInstance.getExpectedRate(
    BAX.address, // srcToken
    ETH_ADDRESS, // destToken
    web3.utils.toWei(new BN(1)), // srcQty
  ));
  stdlog(
    `BAX <-> ETH getExpectedRate() = expectedRate: ${expectedRate}, slippageRate:${slippageRate}`,
  );
  ...
  stdlog(
    `BAX <-> KNC getExpectedRate() = expectedRate: ${expectedRate}, slippageRate:${slippageRate}`,
  );
